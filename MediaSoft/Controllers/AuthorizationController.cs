    ...
    public class AuthorizationController : Controller
    {
        ...
        {           
            if (request.IsPasswordGrantType())
            {
                ...
                //var worker = await _userManagerWorker.FindByIdAsync(request.Username);
                if (user == null)
                {
                ...
                //if (request.UserType == "worker") result = await _signInManagerWorker.CheckPasswordSignInAsync(worker, request.Password, lockoutOnFailure: true);
                if (!result.Succeeded)
                {
                    return BadRequest(new OpenIdConnectResponse
                    {
        ...
        private async Task<AuthenticationTicket> CreateTicketAsync(OpenIdConnectRequest request, Radnik user)
        {
        ...
        //private async Task<AuthenticationTicket> CreateTicketWorkerAsync(OpenIdConnectRequest request, Worker user)
        //{
        //    // Create a new ClaimsPrincipal containing the claims that
        //    // will be used to create an id_token, a token or a code.
